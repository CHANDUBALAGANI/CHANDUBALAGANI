<div align="center">

# 👋 Hi, I'm Chandu Balagani

### 🐍 Python & Django Developer | 🤖 AI Application Builder

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=21&pause=1000&color=4A8FA8&center=true&vCenter=true&width=700&lines=Building+AI-Powered+Web+Applications;Backend+%7C+PostgreSQL+%7C+AI;Turning+Ideas+Into+Real+Projects;Always+Learning+%26+Improving" alt="Typing SVG"/>

<br>

<a href="https://github.com/CHANDUBALAGANI">
<img src="https://img.shields.io/badge/GitHub-CHANDUBALAGANI-181717?style=for-the-badge&logo=github" alt="GitHub"/>
</a>

<a href="https://www.linkedin.com/in/chandu-balagani">
<img src="https://img.shields.io/badge/LinkedIn-Chandu%20Balagani-0A66C2?style=for-the-badge&logo=linkedin" alt="LinkedIn"/>
</a>

</div>

---

# 👨‍💻 About Me

I'm a **Computer Science graduate** focused on **Python, Django, backend development, databases, and AI-powered applications**.

I enjoy building practical applications that solve real-world problems and improving my development skills through hands-on projects.

🎯 **Career Goal:** To start my career as a **Python/Django Developer, Backend Developer, or Software Engineer**.

📍 **Location:** Hyderabad, India

🎓 **Education:** B.Tech in Computer Science

🏫 **Sree Vahini Institute of Science and Technology**

💡 **Interests:** Web Development • Backend Development • Artificial Intelligence • Problem Solving

---

# 🛠️ Technical Skills

### 💻 Languages

<p>
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
</p>

### 🌐 Frontend

<p>
<img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
<img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
<img src="https://img.shields.io/badge/Bootstrap-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
</p>

### ⚙️ Backend

<p>
<img src="https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white"/>
<img src="https://img.shields.io/badge/Django%20ORM-092E20?style=for-the-badge&logo=django&logoColor=white"/>
</p>

### 🗄️ Databases

<p>
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white"/>
<img src="https://img.shields.io/badge/SQLite-003B57?style=for-the-badge&logo=sqlite&logoColor=white"/>
</p>

### 🤖 AI & Cloud

<p>
<img src="https://img.shields.io/badge/Groq%20AI-000000?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white"/>
<img src="https://img.shields.io/badge/Render-46E3B7?style=for-the-badge&logo=render&logoColor=black"/>
</p>

### 🔧 Tools

<p>
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
<img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white"/>
</p>

---

# 🚀 Featured Project

<div align="center">

## 🛍️ ShopMind AI

### AI-Powered E-Commerce Platform

</div>

**ShopMind AI** is my main full-stack project, built with **Django, PostgreSQL, Cloudinary, and Groq AI**.

The application combines traditional e-commerce functionality with an AI shopping assistant that helps users discover products based on their requirements and budget.

### ✨ Key Features

- 🔐 User registration and authentication
- 🛍️ Product browsing and categories
- 🔎 Product search
- 🛒 Shopping cart
- ❤️ Wishlist
- 📦 Checkout and order management
- 📊 Product stock management
- 🖼️ Cloudinary product image storage
- 🤖 AI shopping assistant using Groq API
- 💰 Budget-based product recommendations
- 🗄️ PostgreSQL database
- ☁️ Render deployment

### 🤖 AI Shopping Assistant

The assistant can understand requests such as:

```text
I want a mobile
```

```text
I need a laptop under 80000
```

```text
Show me electronics
```

```text
Which mobile is under 50000?
```

### 🛠️ Tech Stack

`Python` `Django` `PostgreSQL` `HTML` `CSS` `JavaScript` `Bootstrap` `Groq AI` `Cloudinary` `Render`

<div align="center">

<a href="https://shopmind-ai-ykfq.onrender.com">
<img src="https://img.shields.io/badge/🌐%20LIVE%20DEMO-Visit%20ShopMind%20AI-4A8FA8?style=for-the-badge" alt="Live Demo"/>
</a>

<a href="https://github.com/CHANDUBALAGANI/shopmind-ai">
<img src="https://img.shields.io/badge/💻%20SOURCE%20CODE-GitHub-181717?style=for-the-badge&logo=github" alt="Source Code"/>
</a>

</div>

---

# 📊 GitHub Statistics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=CHANDUBALAGANI&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" height="180" alt="GitHub Statistics"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=CHANDUBALAGANI&layout=compact&theme=tokyonight&hide_border=true" height="180" alt="Top Languages"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=CHANDUBALAGANI&theme=tokyonight&hide_border=true" alt="GitHub Streak"/>

</div>

---

# 🏆 Achievements & Activities

- 🏅 Tech Fest 2023 — Volunteer
- 🏐 Represented college in Volleyball
- 💻 Built and deployed Django-based web applications
- 🤖 Built an AI-powered e-commerce application
- ☁️ Deployed a production Django application using Render
- 🗄️ Integrated PostgreSQL, Cloudinary, and external AI APIs

---

# 📚 Currently Learning

- 🚀 Advanced Django
- 🔗 REST API Development
- ⚛️ React Fundamentals
- 🗄️ Advanced PostgreSQL
- 🤖 AI-Powered Application Development

---

# 🎯 Career Goals

I'm currently looking for opportunities as a:

<div align="center">

**🐍 Python Developer**

**🌐 Django Developer**

**⚙️ Backend Developer**

**💻 Software Engineer**

</div>

I'm interested in working on real-world applications where I can contribute, learn from experienced developers, and grow as a software engineer.

---

# 🤝 Open to Collaborate

I'm interested in:

- 🌐 Web applications
- 🤖 AI-powered applications
- 🐍 Python/Django projects
- 🔓 Open-source projects
- 🚀 Startup ideas
- 💼 Software development opportunities

---

# 🎓 Education

### Bachelor of Technology — Computer Science

**Sree Vahini Institute of Science and Technology**

**Branch:** Computer Science

---

# 📈 My Development Journey

<div align="center">

```text
📚 Learn
   ↓
💻 Build
   ↓
🐛 Debug
   ↓
☁️ Deploy
   ↓
🚀 Improve
   ↓
🔁 Repeat
```

</div>

---

# 🌐 Connect With Me

<div align="center">

<a href="https://github.com/CHANDUBALAGANI">
<img src="https://img.shields.io/badge/GitHub-CHANDUBALAGANI-181717?style=for-the-badge&logo=github" alt="GitHub"/>
</a>

<a href="https://www.linkedin.com/in/chandu-balagani">
<img src="https://img.shields.io/badge/LinkedIn-Chandu%20Balagani-0A66C2?style=for-the-badge&logo=linkedin" alt="LinkedIn"/>
</a>

</div>

---

<div align="center">

### 💻 Code. Build. Learn. Repeat. 🚀

⭐ Thanks for visiting my profile!

</div>
